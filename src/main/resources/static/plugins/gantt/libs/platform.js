...
function nearBestPosition(whereId, theObjId, centerOnEl) {
  ...
  if (el) {
    ...
    var left = trueX;
    var top = trueY;
    var barHeight = (/msie/.test(navigator.userAgent.toLowerCase())) ? 45 : 35;
    var barWidth = (/msie/.test(navigator.userAgent.toLowerCase())) ? 20 : 0;
...
function openBlackPopup(url,width,height,onCloseCallBack,iframeId){
  ...
  // bdiv.append("<img src=\"plugins/gantt/res/closeBig.png\" style='cursor:pointer;position:absolute;right:-40px;top:30px;'>");
...
function createBlackPage(width,height,onCloseCallBack){
  ...
  var bg=$("<div>").attr("id","__blackpopup__");
  bg.css({position: 'absolute', top:"0px", left:0, width:'100%', height:'100%',  background: 'rgba(0,0,0,0.5)'});
  bg.append("<div id='bwinPopupd' name='bwinPopupd'></div>");
  bg.bringToFront();

  var ret=bg.find("#bwinPopupd");
  ret.css({position: 'absolute', width: width, left: 0, right: 0, top: '50%', "-moz-box-shadow":'1px 1px 6px #333333', "-webkit-box-shadow":'1px 1px 6px #333333', margin:"10px auto", transform: 'translate(0,-50%)'});

  // var bdiv= $("<div>").css({width:width,position:"relative",height:"0px", textAlign:"right", margin:"auto" });
  // var img=$("<img src='plugins/gantt/res/closeBig.png' style='cursor:pointer;position:absolute;right:-40px;top:5px;' title='close'>");
  // bdiv.append(img);
  // img.click( function(){
  //   bg.trigger("close");
  // });
  ...
  //close call callback
  bg.bind("close",function(){
    bg.find("#bwinPopupd").slideUp(300, function(){
      if (typeof(onCloseCallBack)=="function")
        onCloseCallBack();
    });
    bg.remove();
